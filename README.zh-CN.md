...
**网页看板**（默认指挥主题 · 脱敏演示数据）：左侧命令栏（含视图切换、Agent 家族过滤、释放闲置入口）、系统状态顶栏、红/蓝/绿三列与终端状态条；待确认列为空时仍会自动隐藏。
...
**释放闲置（网页端）**：列出静默超过阈值的候选 Pane，静止 Shell 默认勾选、空闲 Agent 交给你确认，受保护的项目单独收起说明原因。
